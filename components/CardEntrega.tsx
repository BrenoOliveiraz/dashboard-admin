...
export default function CardEntrega({ entrega }: { entrega: any }) {
  return (
    <Link href={`/entregas/${entrega.id}`}>
      ...
      <Card className="shadow-md rounded-xl border border-gray-200 cursor-pointer max-w-md  hover:bg-gray-100">
        ...
        <CardContent className="flex justify-between items-center">
          ...
          <Badge
            variant="secondary"
            className={`text-white px-3 py-1 rounded-lg text-sm ${entrega.status === "Concluído"
                ? "bg-green-500"
